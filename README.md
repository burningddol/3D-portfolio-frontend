...
### 1. 3D 화면 위에 DOM UI를 정확히 오버레이

가장 까다로운 구현이다.  
3D 씬 안의 Screen 메시 위에 `@react-three/drei`의 `<Html>` 컴포넌트를 이용해  
실제 `<iframe>`을 픽셀 단위로 정렬했다.

- `transform` 옵션으로 HTML 요소가 3D 공간의 변환(위치·회전·스케일)을 그대로 따라간다
- `distanceFactor={7}`로 카메라 거리에 따라 자동 스케일링
- `occlude`로 다른 3D 오브젝트에 가려질 때 자동 숨김 처리
- `rotation={[-0.09, 0, 0]}`으로 모니터 기울기를 정밀 보정
...
클릭 이벤트를 3D 캔버스와 DOM 사이에서 상황에 맞게 라우팅한다.
...
`window.postMessage`로 상태를 동기화한다.
...
프레임 업데이트에는 **지수 감쇠 스무딩**을 적용했다.

```ts
const alpha = 1 - Math.exp(-k * delta); // k=7
camera.position.lerp(targetPosition, alpha);
```

일반 `lerp(t)` 방식과 달리 프레임레이트에 무관하게 일정한 감쇠 속도를 보장한다.

---

### 4. 글리치 이펙트 레이어

두 개의 독립된 글리치 레이어가 있다.
...
화면이 포커스될 때는 글리치가 꺼지고, 유휴 상태에서는 켜진다.  
CRT 모니터의 노이즈 느낌을 자연스럽게 연출한다.

---

### 5. 공간 오디오 시스템

씬의 몰입감을 위해 상황별 오디오를 독립적으로 관리한다.
...
모든 오디오는 `useMemo`로 초기화 시 1회만 생성되며,  
재생 시 `currentTime = 0`으로 리셋해 빠른 연속 재생을 지원한다.

---

### 6. Win98 드래그·리사이즈 윈도우

데스크톱 UI의 각 앱 창은 실제 OS처럼 드래그와 리사이즈를 지원한다.
...
**Feature-Sliced Design** 기반으로 3D 레이어와 UI 레이어를 명확히 분리했다.
...
iframe이 완전히 독립된 브라우징 컨텍스트에서 동작한다.
